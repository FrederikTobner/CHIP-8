...
local buildTarget = arg[2] or "CHIP-8"
...
local cmake_x64_command = string.format("cmake -B ../build/x64 -DCMAKE_BUILD_TYPE=%s%s ..", buildType, generator and (" -G " .. generator) or "")
-- This command is only needed on Windows
local vcvars64_command = is_windows and string.format('"%s" && ', vcvars64_path) or ""
os.execute(vcvars64_command .. cmake_x64_command .. (string.format(" && cmake --build ../build/x64 --config %s --target %s", buildType, buildTarget) or ""))

-- We should make the compiler that is used for building the project configurable as well, for now we just use the default
local cmake_x86_command = string.format("cmake -B ../build/x86 -DCMAKE_BUILD_TYPE=%s%s ..", buildType, generator and (" -G " .. generator) or "")
-- This command is only needed on Windows
local vcvars32_command = is_windows and string.format('"%s" && ', vcvars32_path) or ""
os.execute(vcvars32_command .. cmake_x86_command .. (string.format(" && cmake --build ../build/x86 --config %s --target %s", buildType, buildTarget) or ""))
...
-- x64
print("Zipping CHIP-8 x64 ...")
local zip_path = "../build/x64/chip8/main/src/"
if is_windows then
  os.execute("cd " .. zip_path .. "&& cpack -G ZIP --config ../../../CPackConfig.cmake && cpack -G NSIS64 --config ../../../CPackConfig.cmake")
else
  os.execute("cd " .. zip_path .. " && zip -r ../build/chip8-x64.zip " .. zip_path)
end

-- x86
print("Zipping CHIP-8 x86 ...")
zip_path = "../build/x86/chip8/main/src/"
if is_windows then
  os.execute("cd " .. zip_path .. "&& cpack -G ZIP --config ../../../CPackConfig.cmake && cpack -G NSIS --config ../../../CPackConfig.cmake")
else
  os.execute("cd " .. zip_path .. " && zip -r ../build/chip8-x86.zip " .. zip_path)
end